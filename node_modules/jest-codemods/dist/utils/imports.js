...
var _recastHelpers = require('./recast-helpers');
...
function getRequireOrImportName(j, ast, pkg) {
    ...
    findRequires(j, ast, pkg).forEach(p => {
        const variableDeclarationPath = (0, _recastHelpers.findParentVariableDeclaration)(p);
...
function removeDefaultImport(j, ast, pkg) {
    ...
    var _getBodyNode = getBodyNode(j, ast);

    const comments = _getBodyNode.comments;
...
function removeRequireAndImport(j, ast, pkg, specifier) {
    ...
    var _getBodyNode2 = getBodyNode(j, ast);

    const comments = _getBodyNode2.comments;
