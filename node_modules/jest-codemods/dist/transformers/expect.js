...
function splitChainedMatcherPath(j, path) {
    ...
    const pStatement = (0, _recastHelpers.findParentOfType)(path, 'ExpressionStatement');
    ...
    function splitChain(callExpression) {
        ...
        j(path).closest(j.ExpressionStatement).insertAfter(j.expressionStatement(j.callExpression(j.memberExpression(j.callExpression(expectCallExpression.callee, [].concat(_toConsumableArray(expectCallExpression.arguments))), callExpression.callee.property), callExpression.arguments)));
...
function expectTransformer(fileInfo, api, options) {
    ...
    if (!(0, _imports.hasRequireOrImport)(j, ast, EXPECT)) {
        // No expect require/import were found
        return fileInfo.source;
    }

    const expectFunctionName = (0, _imports.getRequireOrImportName)(j, ast, EXPECT) || EXPECT;
    ...
    const getMatchers = () => ast.find(j.MemberExpression, {
        object: {
            type: 'CallExpression',
    ...
    const splitChainedMatchers = () => getMatchers().forEach(path => {
        splitChainedMatcherPath(j, path);
    });

    const updateMatchers = () => getMatchers().forEach(path => {
        if (!standaloneMode) {
...
${keys}.forEach(e => {
  ${matcherNode}
    ...
    const updateSpies = () => {
        ...
        const updateSpyProperty = (path, property) => {
            ...
            if (property.name === 'andCallThrough') {
                const callExpression = (0, _recastHelpers.findParentCallExpression)(path, property.name);
    ...
    const checkForUnsupportedFeatures = () => ast.find(j.MemberExpression, {
        object: {
            name: expectFunctionName
        },
        property: {
            name: _name4 => unsupportedExpectProperties.has(_name4)
        }
    }).forEach(path => {
        logWarning(`"${path.value.property.name}" is currently not supported`, path);
    });
